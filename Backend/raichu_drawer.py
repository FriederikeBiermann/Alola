...
class KKLayout:
    ...
    def initialise_matrices(self):
        ...
        for atom_1 in self.atoms:
            ...
            for atom_2 in self.atoms:
                self.length_matrix[atom_1][atom_2] = self.edge_strength * \
                                                     self.distance_matrix[
                                                         atom_1][atom_2]
                self.spring_strengths[atom_1][atom_2] = self.edge_strength * \
                                                        self.distance_matrix[
                                                            atom_1][
                                                            atom_2] ** -2.0
        ...
        for atom_1 in self.atoms:
            ...
            for atom_2 in self.atoms:
                ...
                self.spring_strengths[atom_1][atom_2] * (
                            (ux - vx) - self.length_matrix[atom_1][atom_2] * (
                                ux - vx) * denom),
                self.spring_strengths[atom_1][atom_2] * (
                            (uy - vy) - self.length_matrix[atom_1][atom_2] * (
                                uy - vy) * denom))

                self.energy_matrix[atom_2][atom_1] = \
                self.energy_matrix[atom_1][atom_2]
    ...
    def get_subgraph_distance_matrix(self, atoms):
        ...
        for atom_1 in atoms:
            for atom_2 in atoms:
                for atom_3 in atoms:
                    if distance_matrix[atom_2][atom_3] > \
...
class Drawer:
    ...
    def prioritise_chiral_bonds(self, chiral_center):
        ...
        subtree_1_size = self.get_subgraph_size(chiral_center.neighbours[0],
                                                {chiral_center})
        subtree_2_size = self.get_subgraph_size(chiral_center.neighbours[1],
                                                {chiral_center})
        subtree_3_size = self.get_subgraph_size(chiral_center.neighbours[2],
                                                {chiral_center})
        subtree_4_size = self.get_subgraph_size(chiral_center.neighbours[3],
                                                {chiral_center})
        ...
        sizes_and_atoms.sort(key=lambda x: (
        x[0], ATOM_PROPERTIES.element_to_atomic_nr[x[1].type]))
        ...
        for neighbour in [size_and_atom[1] for size_and_atom in
                          sizes_and_atoms]:
            if neighbour.type == 'H':
    ...
    def determine_chirality(self, chiral_center):
        ...
        if tuple(priority) in get_chiral_permutations(
                chiral_center.neighbours):
    ...
    def plot_chiral_bond(self, orientation, chiral_center, line, ax, midpoint):
        ...
        for halfline in halflines:
            ...
            if orientation == 'front':
                ...
                self.plot_chiral_bond_front(bond_wedge, ax,
                                            color=halfline.atom.draw.colour)
            else:
                bond_lines = halfline.get_bond_wedge_back(
                    self.options.chiral_bond_width, chiral_center)
                self.plot_chiral_bond_back(bond_lines, ax,
                                           color=halfline.atom.draw.colour)
                        ...
                        if len(neighbours_2) == 1:
                            ...
                            if len(neighbour_1.draw.rings) == 1 and len(
                                ...
                                if neighbour_1.draw.rings[0] != \
                                    ...
                                    self.rotate_subtree(neighbour_1, atom_2,
                                                        angle_1,
                                                        atom_2.position)
                                    self.rotate_subtree(neighbour_2, atom_2,
                                                        angle_2,
                                                        atom_2.position)
    ...
    def create_next_bond(self, atom, previous_atom=None, angle=0.0,
        ...
        if not skip_positioning:
            if not previous_atom:
                ...
                if previous_atom.draw.bridged_ring == None and len(
                        previous_atom.draw.rings) > 1:
                    for neighbour in neighbours:
                        if len(set(neighbour.draw.rings) & set(
                                previous_atom.draw.rings)) == len(
                                previous_atom.draw.rings):
                ...
                if not joined_vertex:
                    for neighbour in neighbours:
                        ...
                        if neighbour.draw.positioned and self.atoms_are_in_same_ring(
                            ...
                            position.add(Vector.subtract_vectors(
                                neighbour.draw.position,
                                previous_atom.draw.position))
                    ...
                    position.rotate_around_vector(math.pi,
                                                  previous_atom.draw.position)
        ...
        if atom.draw.bridged_ring != None:
            ...
            if not next_ring.positioned:
                ...
                scalar = Polygon.find_polygon_radius(self.options.bond_length,
                                                     len(next_ring.members))
        ...
        # If the atom is part of a ring: position the entire ring
            ...
            if not next_ring.positioned:
                ...
                radius = Polygon.find_polygon_radius(self.options.bond_length,
                                                     len(next_ring.members))
        ...
        # If the atom is not part of a ring, position just the atom
            ...
            if len(neighbours) == 1:
                ...
                if previous_atom:
                    previous_bond = self.structure.bond_lookup[previous_atom][
                        atom]

                if current_bond.type == 'triple' or (
                        previous_bond and previous_bond.type == 'triple') or \
                        (
                                current_bond.type == 'double' and previous_bond and previous_bond.type == 'double' and
                                previous_atom and len(
                            previous_atom.draw.rings) == 0 and
                    ...
                    self.create_next_bond(next_atom, atom,
                                          previous_angle + next_atom.draw.angle)
                    ...
                    distance_1 = proposed_vector_1.get_squared_distance(
                        centre_of_mass)
                    distance_2 = proposed_vector_2.get_squared_distance(
                        centre_of_mass)
                    ...
                    if previous_atom and len(
                            previous_atom.drawn_neighbours) > 3:
                        if a > 0:
                    ...
                    if previous_atom:
                        ...
                        if bond.type == 'double' and bond.chiral:
                            ...
                            if previous_previous_atom:
                                ...
                                configuration = \
                                bond.chiral_dict[previous_previous_atom][
                                    next_atom]
                ...
                if previous_atom:
                    subgraph_3_size = self.get_subgraph_size(previous_atom,
                                                             {atom})
                ...
                if cis_bond.type == 'single' and trans_bond.type == 'single':
                    ...
                    if previous_atom:
                        previous_bond = self.structure.bond_lookup[atom][
                            previous_atom]
                        ...
                        if previous_bond.type == 'double' and previous_bond.chiral:
                            configuration_cis_atom = previous_bond.chiral_dict[
                                previous_atom.draw.previous_atom][cis_atom]
                            if configuration_cis_atom == 'cis':
                                trans_atom.draw.angle = -a
                                cis_atom.draw.angle = a

                self.create_next_bond(trans_atom, atom,
                                      previous_angle + trans_atom.draw.angle,
                                      previous_branch_shortest)
                self.create_next_bond(cis_atom, atom,
                ...
                self.create_next_bond(straight_atom, atom,
                                      previous_angle + straight_atom.draw.angle)
                self.create_next_bond(left_atom, atom,
                                      previous_angle + left_atom.draw.angle)
                self.create_next_bond(right_atom, atom,
                                      previous_angle + right_atom.draw.angle)
                ...
                self.create_next_bond(atom_1, atom,
                                      previous_angle + atom_1.draw.angle)
                self.create_next_bond(atom_2, atom,
                                      previous_angle + atom_2.draw.angle)
                self.create_next_bond(atom_3, atom,
                                      previous_angle + atom_3.draw.angle)
                self.create_next_bond(atom_4, atom,
                                      previous_angle + atom_4.draw.angle)
    ...
    def bond_is_rotatable(self, bond):
        ...
        if len(bond.atom_1.drawn_neighbours) == 1 or len(
                bond.atom_2.drawn_neighbours) == 1:
            return False

        if bond.atom_1.draw.rings and \
                bond.atom_2.draw.rings and \
                len(set(bond.atom_1.draw.rings).intersection(
                    set(bond.atom_2.draw.rings))) > 0:
            return False
    ...
    def resolve_primary_overlaps(self):
        ...
        for ring in self.rings:
            for atom in ring.members:
                ...
                if len(non_ring_neighbours) > 1 or \
                        (len(non_ring_neighbours) == 1 and len(
                            atom.draw.rings) == 2):
        ...
        for overlap in overlaps:
            ...
            if len(branches_to_adjust) == 2:
                ...
                angle = (2 * math.pi - self.id_to_ring[
                    rings[0]].get_angle()) / 6.0
                ...
                self.rotate_subtree(atom_1, root, -2.0 * angle,
                                    root.draw.position)
                self.rotate_subtree(atom_2, root, 2.0 * angle,
                                    root.draw.position)
                ...
                if len(atom.drawn_neighbours) <= 1:
                    ...
                    if len(drawn_neighbours) <= 1:
                        if not closest_atom.draw.previous_position:
                            closest_position = drawn_neighbours[
                                0].draw.position
                        else:
                    ...
                    if not atom.draw.previous_position:
                        atom_previous_position = atom.drawn_neighbours[
                            0].draw.position
                    else:
    ...
    def get_overlap_score(self):
        ...
        for i, atom_1 in enumerate(self.drawn_atoms):
            for j in range(i + 1, len(self.drawn_atoms)):
                ...
                distance = Vector.subtract_vectors(atom_1.draw.position,
                                                   atom_2.draw.position).get_squared_length()
                if distance < self.options.bond_length_squared:
                    weight = (self.options.bond_length - math.sqrt(
                        distance)) / self.options.bond_length
                    total += weight
    ...
    def create_ring(self, ring, center=None, start_atom=None,
                    previous_atom=None):
        ...
        if start_atom:
            starting_angle = Vector.subtract_vectors(start_atom.draw.position,
                                                     center).angle()
        ...
        if ring.bridged:
            KKLayout(self.structure, ring.members, center, start_atom,
                     self.options.bond_length, self.options.kk_threshold,
            ...
            ring.set_member_positions(self.structure, start_atom,
                                      previous_atom, center, starting_angle,
                                      radius, angle)
        ...
        for neighbour_id in ordered_neighbour_ids:
            ...
            atoms = list(RingOverlap.get_vertices(self.ring_overlaps, ring.id,
                                                  neighbour.id))
            ...
            if len(atoms) == 2:
                ...
                midpoint = Vector.get_midpoint(atom_1.draw.position,
                                               atom_2.draw.position)
                normals = Vector.get_normals(atom_1.draw.position,
                                             atom_2.draw.position)
                ...
                distance_to_center_1 = Vector.subtract_vectors(center, normals[
                    0]).get_squared_length()
                distance_to_center_2 = Vector.subtract_vectors(center, normals[
                    1]).get_squared_length()

                if distance_to_center_2 > distance_to_center_1:
                    next_center = normals[1]

                position_1 = Vector.subtract_vectors(atom_1.draw.position,
                                                     next_center)
                position_2 = Vector.subtract_vectors(atom_2.draw.position,
                                                     next_center)

                if position_1.get_clockwise_orientation(
                        position_2) == 'clockwise':
                    if not neighbour.positioned:
                        self.create_ring(neighbour, next_center, atom_1,
                                         atom_2)

                else:
                    if not neighbour.positioned:
                        self.create_ring(neighbour, next_center, atom_2,
                                         atom_1)
                ...
                atom = atoms[0]
                next_center = Vector.subtract_vectors(center,
                                                      atom.draw.position)
    ...
    def define_rings(self):
        ...
        for ring in self.rings:
            neighbouring_rings = find_neighbouring_rings(self.ring_overlaps,
                                                         ring.id)
    ...
    def hide_hydrogens(self):
        ...
        for atom in self.structure.graph:
            ...
            if len(
                    neighbour.draw.rings) < 2 and neighbour.draw.bridged_ring == None and \
                    neighbour.draw.bridged_ring != None and len(
                neighbour.draw.original_rings) < 2:
    ...
    def get_bridged_ring_subrings(self, ring_id, involved_ring_ids):
        ...
        for neighbour_id in ring.neighbouring_rings:
            if neighbour_id not in involved_ring_ids and neighbour_id != ring_id and \
                    rings_connected_by_bridge(self.ring_overlaps, ring_id,
                                              neighbour_id):
    ...
    def create_bridged_ring(self, involved_ring_ids):
        ...
        for neighbour_id in neighbours:
            ring_overlaps = self.get_ring_overlaps(neighbour_id,
                                                   involved_ring_ids)
    ...
    def get_ring_overlaps(self, ring_id, ring_ids):
        ...
        for ring_overlap in self.ring_overlaps:
            for id in ring_ids:
                if (
                        ring_overlap.ring_id_1 == ring_id and ring_overlap.ring_id_2 == id) or \
                        (
    ...
    def is_part_of_bridged_ring(self, ring_id):
        for ring_overlap in self.ring_overlaps:
            if ring_overlap.involves_ring(
                    ring_id) and ring_overlap.is_bridge():
                return True
