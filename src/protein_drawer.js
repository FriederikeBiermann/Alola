...
Proteiner.drawClusterSVG = (function(cluster, height = 90) {
    ...
    if (cluster.hasOwnProperty("orfs")) {
        ...
        for (var i in cluster.orfs) {
            ...
            innerContainer.style.width = String(scale(orf.end - orf.start)) +
                "px"
            document.getElementById('protein_container')
                .appendChild(innerContainer)
                .setAttribute("draggable", "true");
            document.getElementById('innerProteinContainer' + orf.locus_tag)
                .setAttribute("class", "box");
            document.getElementById('innerProteinContainer' + orf.locus_tag)
                .appendChild(innerDropdownContainer);
            document.getElementById("innerDropdownContainer" + orf.locus_tag)
                .setAttribute("class", "dropdown");
            document.getElementById('innerDropdownContainer' + orf.locus_tag)
                .appendChild(innerDropdownButton);
            document.getElementById("innerDropdownButton" + orf.locus_tag)
                .setAttribute("class", "dropbtn");
            document.getElementById('innerDropdownContainer' + orf.locus_tag)
                .appendChild(innerDropdownContent);
            document.getElementById("innerDropdownContent" + orf.locus_tag)
            ...
            for (geneIndex = 0; geneIndex < geneMatrix.length; geneIndex++) {
                if (geneMatrix[geneIndex].id == orf.locus_tag) {
                    for (let optionIndex = 0; optionIndex < geneMatrix[
                            geneIndex].options.length; optionIndex++) {
                        let optionContent =
            ...
            var draw = SVG(innerDropdownButton)
                .size(String(scale(orf.end - orf.start) + 10) + "px",
                    height)
                .group();
            ...
            $(pol.node)
                .mouseover({
                    ...
                    Proteiner.showToolTip("ORF: " + handler.data.orf
                        .locus_tag + "<br/>", handler);
                    $(handler.target)
            ...
            if (orf.hasOwnProperty("domains")) {
                ...
                for (var j in orf.domains) {
                    ...
                    points = Proteiner.getDomainPoints(domain, orf,
                    ...
                    $(dom.node)
                        .mouseover({
                            ...
                            for (let domainIndex = 0; domainIndex <
                                geneMatrix[geneIndex].domains.length; domainIndex++
                            ) {
                                if (geneMatrix[geneIndex].domains[
                                        domainIndex].start ==
                                    handler.data.domain.start) {
                            ...
                            Proteiner.showToolTip("Domain: " +
                                handler.data.domain.abbreviation +
                                " (" + handler.data.domain.type +
                                ")" + "<br/>" + start + " - " +
                                end, handler);
                            $(handler.target)
                                .css("stroke-width", "3px");
...
Proteiner.showToolTip = (function(html, handler) {
    var divTooltip = ""
    if (html.includes("<a")) {
        divTooltip = $("#" + Proteiner.tooltip_id_protein);
    }
    else {
        divTooltip = $("#" + Proteiner.tooltip_id);
    }
